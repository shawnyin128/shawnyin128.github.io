...
Why do the expectations differ? **SFT is supervised by a target trajectory**, so it maximizes that trajectory's probability and averages over $y\sim q(\cdot\mid x)$. **On-policy RL has no target trajectory to imitate**, so the policy samples its own trajectories and maximizes their expected reward under $y\sim\pi_\theta(\cdot\mid x)$. This also replaces SFT's fixed coefficient $1$ with the reward-derived advantage $A_t$.
